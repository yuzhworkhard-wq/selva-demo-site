  ...
  return `
    <div class="chart-card">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
        ...
        <span style="font-size:11px; color:#666;">按任务实际调用的视频模型统计</span>
      </div>
      <div style="display:flex; flex-direction:column; gap:10px;">
...
// ===== 素材成功率（下载采用率） =====
// Demo 口径：按【生成日期】归集当天数据；成功率 = 当天被本人下载采用条数 ÷ 当天生成成功条数。
//   · 生成成功 = 任务真正产出、状态 done 的视频（同 “生成视频” 卡口径）。
//   · 同一条视频去重：重复下载同一条只计一次采用。
//   · 组长下载组员的视频不计入本人采用（只认 “生成人本人” 的下载）。
// 本前端 demo 无真实下载埋点，“是否被本人采用” 用视频身份的确定性散列模拟，
// 保证 概览卡 / 每日明细 / 成员明细 三处口径完全一致（各行之和 = 概览）。
function statVideoKey(v) {
  return (v.taskId || '') + '::' + (v.name || '');
}
function isStatVideoAdopted(v) {
  // 确定性判定：由视频身份散列得到稳定布尔值，可视作 “去重后是否被本人下载采用”
  const key = statVideoKey(v);
  let h = 2166136261;
...
function computeStatsSuccess(videos) {
  ...
  let adopted = 0;
  list.forEach(v => { if (isStatVideoAdopted(v)) adopted++; });
  const rate = generated ? Math.round((adopted / generated) * 100) : 0;
  return { generated, adopted, rate };
}
function successRateColor(rate, generated) {
  if (!generated) return '#666';
...
function computeMemberCounts(userList, producedVideos) {
  // 按生成人（任务 owner）归集所选范围内的产出视频，供成员概览展示 生成视频数 + 素材成功率。
  // 与个人/团队统计口径一致：生成 = 任务产出的 done 视频，成功率 = 本人下载采用（去重）/ 生成。
  const byOwner = new Map();
  (producedVideos || []).forEach(v => {
    const owner = getTaskOwnerUser(v.task);
    if (!owner) return;
    if (!byOwner.has(owner.id)) byOwner.set(owner.id, []);
    byOwner.get(owner.id).push(v);
  });
  return userList.map(u => {
    const s = computeStatsSuccess(byOwner.get(u.id) || []);
    return { user: u, generated: s.generated, adopted: s.adopted, rate: s.rate };
  });
}

function buildTeamMemberOverview(memberList, producedVideos, filterHtml = '') {
...
// ===== 爆款视频库（左侧菜单独立页）=====
// 筛选条和卡片在同一份文档流里，跟着 .main 一起滚，不要 sticky / 不要再套一层 overflow。
const VL_CHANNELS = ['全部', 'Facebook系', 'Google系', 'TikTok', 'X(Twitter)', 'UnityAds', 'AppLovin'];
const VL_COUNTRIES = ['全部国家/地区', '美国', '巴西', '印尼', '墨西哥', '日本'];
const VL_LANGS = ['全部文案语言', '英语', '葡萄牙语', '西班牙语', '印尼语', '日语'];
const VL_ATTRS = ['全部素材属性', '真人出镜', 'UGC', '动画', '产品特写'];
const VL_QUICK = ['近7天新投', '近30天爆款', '近30天初投视频'];
const VL_RANGES = ['7天', '30天', '90天', '近1年'];
const VL_SORTS = ['最新创意', '最后看见', '人气值'];
const VL_APPS = [
  { app: 'TikTok', dev: 'TikTok Pte. Ltd.', cover: 'clone/showcase/sarah.jpg', channel: 'TikTok' },
  { app: 'MusicBash', dev: 'Bash Labs Inc.', cover: 'clone/showcase/jonas.jpg', channel: 'Facebook系' },
  { app: 'Reading Eggs', dev: 'Blake eLearning', cover: 'clone/showcase/chloe.jpg', channel: 'Google系' },
  { app: 'MixReels', dev: 'Reel Labs', cover: 'clone/showcase/elena.jpg', channel: 'TikTok' },
  { app: 'CashTok', dev: 'Northstar Apps', cover: 'clone/showcase/mei.jpg', channel: 'AppLovin' },
  { app: 'PuzzleGo', dev: 'Arcade Nine', cover: 'clone/frames/frame_09.jpg', channel: 'UnityAds' },
  { app: 'GlowSkin', dev: 'Lumen Beauty', cover: 'clone/showcase/zoe.jpg', channel: 'Facebook系' },
  { app: 'NightChat', dev: 'Polar Social', cover: 'clone/showcase/victor.jpg', channel: 'X(Twitter)' },
  { app: 'CoinFarm', dev: 'Harvest Play', cover: 'clone/frames/frame_04.jpg', channel: 'Google系' },
  { app: 'BeatRush', dev: 'Pulse Studio', cover: 'clone/showcase/kai.jpg', channel: 'TikTok' },
  { app: 'StudyBuddy', dev: 'Owl & Oak', cover: 'clone/showcase/priya.jpg', channel: 'Facebook系' },
  { app: 'MiniDrama', dev: 'Shortform Co.', cover: 'clone/showcase/owen.jpg', channel: 'AppLovin' },
];
const VL_COUNTRY_CYCLE = ['美国', '巴西', '印尼', '墨西哥', '日本'];
const VL_LANG_CYCLE = ['英语', '葡萄牙语', '西班牙语', '印尼语', '日语'];
const VL_ATTR_CYCLE = ['真人出镜', 'UGC', '动画', '产品特写'];
const VIRAL_ADS = Array.from({ length: 36 }, (_, i) => {
  const base = VL_APPS[i % VL_APPS.length];
  return {
    id: 'vl-' + i,
    app: base.app,
    dev: base.dev,
    ...
    country: VL_COUNTRY_CYCLE[i % VL_COUNTRY_CYCLE.length],
    language: VL_LANG_CYCLE[i % VL_LANG_CYCLE.length],
    attr: VL_ATTR_CYCLE[i % VL_ATTR_CYCLE.length],
    duration: 11 + (i % 27),
    popularity: 9800 - i * 170,
    runDays: 3 + (i * 3) % 54,
    lastSeen: 1 + (i % 18),
    firstSeen: 2 + (i % 42),
    impressions: (60 + i * 11) + '万',
    heat: 99 - i,
  };
});

function setViralLibFilter(key, value) {
  if (key === 'quick' && viralLibFilter.quick === value) viralLibFilter.quick = '';
  else viralLibFilter[key] = value;
  renderViralLibrary();
}

function getViralLibraryItems() {
  const f = viralLibFilter;
  const rangeDays = f.range === '7天' ? 7 : f.range === '30天' ? 30 : f.range === '90天' ? 90 : 365;
  let list = VIRAL_ADS.filter(item => {
    if (f.channel !== '全部' && item.channel !== f.channel) return false;
    if (f.country !== '全部国家/地区' && item.country !== f.country) return false;
    if (f.language !== '全部文案语言' && item.language !== f.language) return false;
    if (f.attr !== '全部素材属性' && item.attr !== f.attr) return false;
    if (item.lastSeen > rangeDays) return false;
    if (f.quick === '近7天新投' && item.firstSeen > 7) return false;
    if (f.quick === '近30天爆款' && item.heat < 80) return false;
    if (f.quick === '近30天初投视频' && item.firstSeen > 30) return false;
    return true;
  });
  if (f.viewMode === '广告主') {
    const seen = new Set();
  ...
  if (f.sort === '最后看见') list = [...list].sort((a, b) => a.lastSeen - b.lastSeen);
  else if (f.sort === '人气值') list = [...list].sort((a, b) => b.popularity - a.popularity);
  else list = [...list].sort((a, b) => a.firstSeen - b.firstSeen);
  return list;
}

function vlPills(label, options, current, key) {
  return `<div class="vl-row">
    <span class="vl-row-label">${label}</span>
    <div class="vl-pills">
      ${options.map(option => `<button type="button" class="vl-pill${option === current ? ' on' : ''}" onclick="setViralLibFilter('${key}','${option}')">${option}</button>`).join('')}
    </div>
  </div>`;
}
...
function renderViralLibrary() {
  ...
  if (!container) return;
  const f = viralLibFilter;
  const list = getViralLibraryItems();
  const select = (key, options, value) => `<select class="vl-select" onchange="setViralLibFilter('${key}', this.value)">
    ${options.map(option => `<option value="${option}" ${option === value ? 'selected' : ''}>${option}</option>`).join('')}
  </select>`;

  const cards = list.map(item => f.layout === 'list'
    ? `<button type="button" class="vl-list-row" onclick="toast('已打开「${item.app}」投放详情（原型演示）')">
        <img src="${item.cover}" alt="" class="vl-list-thumb">
        <span class="vl-list-main"><b>${item.app}</b><em>${item.dev} · ${item.channel}</em></span>
        <span>人气 ${item.popularity}</span>
        <span>投放 ${item.runDays} 天</span>
        <span>最后看见 ${item.lastSeen} 天前</span>
      </button>`
    : `<button type="button" class="vl-card" onclick="toast('已打开「${item.app}」投放详情（原型演示）')">
        <span class="vl-card-head">
          <span class="vl-app-mark">${item.app.slice(0, 1)}</span>
          <span class="vl-card-names"><b>${item.app}</b><em>${item.dev}</em></span>
        </span>
        <span class="vl-thumb">
          <img src="${item.cover}" alt="">
          <span class="vl-play">▶</span>
          <span class="vl-dur">${item.duration}s</span>
          <span class="vl-dates">${item.firstSeen}天前 → ${item.lastSeen}天前</span>
        </span>
        <span class="vl-card-meta"><span class="vl-ch">${item.channel}</span><span>${item.language}</span></span>
        <span class="vl-stats">
          <span><em>人气值</em><b>${item.popularity}</b></span>
          <span><em>投放天数</em><b>${item.runDays}</b></span>
          <span><em>最后看见</em><b>${item.lastSeen}天前</b></span>
        </span>
        <span class="vl-sub"><span>展示估值 ${item.impressions}</span><span>热度 ${item.heat}</span></span>
      </button>`
  ).join('');

  container.innerHTML = `
    <div class="page-title">爆款视频库</div>
    <div class="vl-filters">
      ${vlPills('渠道', VL_CHANNELS, f.channel, 'channel')}
      <div class="vl-row">
        <span class="vl-row-label">广告信息</span>
        <div class="vl-pills">
          ${select('country', VL_COUNTRIES, f.country)}
          ${select('language', VL_LANGS, f.language)}
          ${select('attr', VL_ATTRS, f.attr)}
          <button type="button" class="vl-link" onclick="toast('高级筛选为原型演示')">高级筛选 ▾</button>
        </div>
      </div>
      ${vlPills('快捷操作', VL_QUICK, f.quick, 'quick')}
      <div class="vl-row">
        <span class="vl-row-label">时间</span>
        <div class="vl-pills">
          ${VL_RANGES.map(option => `<button type="button" class="vl-pill${option === f.range ? ' on' : ''}" onclick="setViralLibFilter('range','${option}')">${option}</button>`).join('')}
          <span class="vl-count">共找到 ${list.length} 个相关广告</span>
        </div>
      </div>
      <div class="vl-toolbar">
        <div class="vl-pills">
          <span class="vl-row-label">排序</span>
          ${VL_SORTS.map(option => `<button type="button" class="vl-pill${option === f.sort ? ' on' : ''}" onclick="setViralLibFilter('sort','${option}')">${option}</button>`).join('')}
        </div>
        <div class="vl-pills">
          ${['广告', '素材', '广告主'].map(option => `<button type="button" class="vl-pill${option === f.viewMode ? ' on' : ''}" onclick="setViralLibFilter('viewMode','${option}')">${option}</button>`).join('')}
          <button type="button" class="vl-pill${f.layout === 'grid' ? ' on' : ''}" onclick="setViralLibFilter('layout','grid')">宫格</button>
          <button type="button" class="vl-pill${f.layout === 'list' ? ' on' : ''}" onclick="setViralLibFilter('layout','list')">列表</button>
        </div>
      </div>
    </div>
    <div class="${f.layout === 'list' ? 'vl-list' : 'vl-grid'}">
      ${cards || '<div class="vl-empty">当前筛选下没有广告，试试放宽条件</div>'}
    </div>
  `;
}
